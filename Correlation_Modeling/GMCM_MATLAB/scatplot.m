function out = scatplot(x,y,method,radius,N,n,po,ms, my_colormap)
...
switch po
    case {1,2}
        hs = gsp(x,y,ddf,ms, my_colormap);
        out.hs = hs;
        colorbar_pro = colorbar;
        colorbar_pro = colorbar_pro.TickLabels;
        if po==2
            [c,h] = contour(xi,yi,zif, 'LineWidth', 1, 'color', 'k');
            out.c = c;
            out.h = h;
        end %if
        colorbar_pro2 = colorbar;
        colorbar_pro2 = colorbar_pro2.Ticks;
        colorbar('Ticks', colorbar_pro2, 'TickLabels', colorbar_pro);
    case {3,4}
        hs = gsp(x,y,dd,ms, my_colormap);
        out.hs = hs;
        colorbar_pro = colorbar;
        colorbar_pro = colorbar_pro.TickLabels;
        if po>3
            [c,h] = contour(xi,yi,zi, 'LineWidth', 1, 'color', 'k');
            out.c = c;
        end %if
        colorbar_pro2 = colorbar;
        colorbar_pro2 = colorbar_pro2.Ticks;
...
return
%~~~~~~~~~~ Graf Scatter Plot ~~~~~~~~~~~
function varargout = gsp(x,y,c,ms, my_colormap)
